...
$(document).ready(function () {
  ...
  function loadSettingsCategory(category) {
    ...
    $.ajax({
      ...
      success: function (response) {
        try {
          if (response.success) {
            var keys = response.data ? Object.keys(response.data) : [];
            var selectedCategory =
              response.data && response.data[category]
                ...
                : null;
            if (selectedCategory) {
              displaySettingsCategory(response.data[selectedCategory]);
              $("#testEmailConfig").toggle(selectedCategory === "email");
              hideLoading();
            } else {
              hideLoading();
              showAlert(
          ...
          showAlert(
            "Error",
            "An error occurred while displaying settings",
            "error"
          );
        }
      },
  ...
  function displaySettingsCategory(categoryData) {
    try {
      console.log("displaySettingsCategory called with:", categoryData);
      ...
      if (Array.isArray(categoryData.settings)) {
        categoryData.settings.forEach(function (setting) {
          html += generateSettingField(setting);
        });
      } else {
        console.error("No settings array found in categoryData:", categoryData);
      }
      ...
      console.log("Settings HTML inserted, length:", html.length);
      $("#settingsTitle").text(categoryData.display_name);
      $("#saveAllSettings").show();
    } catch (err) {
      console.error("Error in displaySettingsCategory:", err);
      showAlert("Error", "An error occurred while rendering settings", "error");
    }
  }
...
>>>>>>> Stashed changes
            <div class="form-group">
                <label for="${setting.setting_key}">${formatSettingKey(
      setting.setting_key
    )}</label>
        `;
    ...
    schedules.forEach(function (schedule) {
      ...
      html += `
                <tr>
                    ...
                    <td>${statusBadge}</td>
                    <td>
                        <button class="btn btn-sm btn-primary" onclick="runMaintenanceTask(${
                          schedule.id
                        })">
                            <i class="fas fa-play"></i>
                        </button>
                        <button class="btn btn-sm btn-warning" onclick="editMaintenanceTask(${
                          schedule.id
                        })">
      ...
      html += `
                <div class="card mb-3">
                    ...
                    <div class="card-body">
                        ...
                        <button class="btn btn-sm btn-primary" onclick="editEmailTemplate(${
                          template.id
                        })">
  ...
  function optimizeDatabase() {
    if (
      !confirm(
        "Are you sure you want to optimize the database? This may take some time."
      )
    ) {
      return;
    }
  ...
  function getStatusBadge(status) {
    ...
    return (
      badges[status] || '<span class="badge badge-secondary">Unknown</span>'
    );
  }
  ...
  function hideLoading() {
    console.log("hideLoading called (Sneat forced)");
    $("#loadingModal").modal("hide");
    // Force remove modal backdrop and classes
    setTimeout(function() {
  ...
  function showAlert(title, message, type = "info") {
    ...
    if (!$(".toast-container").length) {
      $("body").append(
        '<div class="toast-container position-fixed top-0 right-0 p-3" style="z-index: 1060;"></div>'
      );
    }
  ...
  window.editMaintenanceTask = function (scheduleId) {
    ...
    showAlert(
      "Info",
      "Edit maintenance task functionality coming soon",
      "info"
    );
  };
